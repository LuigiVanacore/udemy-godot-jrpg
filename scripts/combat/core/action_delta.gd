...
# ATTENZIONE: meglio Variant perché i tuoi id possono essere StringName, int, ecc.
var id: int
var kind: int = Kind.CUSTOM
...
# --------- Factory methods ---------
static func hp(target_id: int, amount: int) -> ActionDelta:
...
static func mp(target_id: int, amount: int) -> ActionDelta:
	var d := ActionDelta.new()
	d.id = target_id
	d.kind = Kind.MP
	d.value = amount
...
static func status_add(target_id: int, status_key: StringName, stacks: int = 1, duration_turns: int = -1, source: Variant = null, overwrite: bool = false) -> ActionDelta:
	var d := ActionDelta.new()
	d.id = target_id
...
static func status_remove(target_id: int, status_key: StringName) -> ActionDelta:
	var d := ActionDelta.new()
	d.id = target_id
...
static func consume_item(caster_id: int, item_id: StringName, amount: int = 1) -> ActionDelta:
	var d := ActionDelta.new()
	d.id = caster_id
...
static func give_item(target_id: int, item_id: StringName, amount: int = 1) -> ActionDelta:
	var d := ActionDelta.new()
	d.id = target_id
...
func to_dict() -> Dictionary:
	match kind:
		Kind.HP:
			return {"id": id, "hp": int(value)}
		Kind.MP:
			return {"id": id, "mp": int(value)}
		Kind.STATUS_ADD:
			# rappresento lo status come dizionario semplice
			var out := {"id": id, "status_add": value}
			for k in meta.keys():
				out[k] = meta[k]
			return out
		Kind.STATUS_REMOVE:
			return {"id": id, "status_remove": value}
		Kind.CONSUME_ITEM:
			return {"id": id, "consume_item": value, "amount": int(meta.get("amount", 1))}
		Kind.GIVE_ITEM:
			return {"id": id, "give_item": value, "amount": int(meta.get("amount", 1))}
		_:
			return {"id": id, "kind": kind, "value": value, "meta": meta}
